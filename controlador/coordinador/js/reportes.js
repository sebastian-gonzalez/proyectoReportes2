...
$(document).ready(function() {
	...
	$.ajax({
		url: "../../controlador/coordinador/reportes.php",
		type: "POST",
		datatype: "json",
		data: {
		...
		success: function(data) {
			//console.log(data);
			camposFicha = JSON.parse(data);
			console.log(camposFicha);

			if (camposFicha.length > 0) {
				
				for (var i = 0; i < camposFicha.length; i++) {
					
					llenarTablaReporte(camposFicha[i]);
				}
				
				cargarDataTablesReportes("#tablaFichas", true, datos);
			...
			//console.log(JSON.parse(camposFicha));
	...
	/** ejecutarAjax("/proyecto/controlador/coordinador/reportesController.php", opcion, function(dataJson) { 	
	  ...
	  buttons:[
  {
	...
	extend: 'pdfHtml5',
	  ...
	  columns: [
		{ data: "id_ficha",className:"hide_column"},
		{ data: "titulo_ficha" },
...
function ejecutarAjax(servlet, datos, callBackFunc, asincrono = true) {

	//datos.usuario = JSON.parse(localStorage.getItem("usuario"));
	...
	$.ajax({
		...
		//Define si es una peticion asincrona
		async: asincrono,
		...
		error: function(request, status, er) {
			ocultarLoader();
			//console.log("Error"+request.responseText+" Estado: "+status+" Error:"+er);
			//imprimirMensaje("Mensaje","Error", "Error", "<b>Error: </b>"+request.responseText+"<br/><b> Estado: </b>"+status+"<br> <b>Error: </b>"+er, "", "");
			console.log("Mensaje", "Error", "Error", "" || "Se ha presentado un error, por favor intente más tarde.", "", "");
		},
		...
		beforeSend: function() {
			//console.log("["+servlet+"] Procesando...");
...
function cargarDataTablesReportes(tabla, completo, data) {
...
function mostrarLoader() {
	//var loader = "<div nombre='loader' id='loader'> <div id='spin'></div></div>";
	//$("#contenedorPrincipal").append(loader);

	// if ($("[nombre=loader]").length > 0 ) {
	// 	var loader = "<div nombre='loader' style='display: none !important;' id='loader'> <div id='spin'></div></div>";
	// }else{
	// 	var loader = "<div nombre='loader' id='loader'> <div id='spin'></div></div>";
	// }
}
...
function llenarTablaReporte(camposFicha) {
	...
	/** const filteredCategories = [];
		...
		camposFicha.forEach(category => {
		if (!filteredCategories.find(cat => cat.fk_id_ficha == category.fk_id_ficha && cat.descripcion_campo == category.descripcion_campo)) {
			const { fk_id_ficha, descripcion_campo } = category;
			filteredCategories.push({ fk_id_ficha, descripcion_campo });
		}
	}); 
	...
	console.log(camposFicha);
	var longitud = camposFicha.length;

	for (var i = 0; i < camposFicha.length; i++) {

		if (camposFicha[(i + 1)] != undefined) {

			if (camposFicha[i].fk_id_ficha == camposFicha[(i + 1)].fk_id_ficha) {
				
				//console.log("Posicion anterior", camposFicha[i].fk_id_ficha);
				//console.log("Posicion siguiente", camposFicha[(i + 1)].fk_id_ficha);

				if (camposFicha[i].descripcion_campo == "Objetivo general") {

					
					valor_campo = valor_campo + "<ul><li>" + camposFicha[i].valor_campo + "</li><ul>";
					console.log(valor_campo);
				
				} else if(camposFicha[i].descripcion_campo == "Objetivo especifico"){
					
					//console.log(camposFicha[i].descripcion_campo);
				}
