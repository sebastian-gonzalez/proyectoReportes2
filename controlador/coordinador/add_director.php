...
if (isset($_POST['add_director'])) {
     ...
     foreach ($_POST['id_lista_usuario_ev'] as $key => $value) {
          ...
          $consulta_validacion = "SELECT * FROM lista_ficha WHERE id_lista_ficha =$id_lista_ficha AND (id_lista_usuario=$id_lista_usuario AND id_rol_ficha = 2 AND activo IS null)";
          $resultado_vali = $conexion->prepare($consulta_validacion);
          $data_vali = $resultado_vali->execute();

          if ($resultado_vali->fetchColumn() > 0) {
               echo '<script language="javascript">alert("La ficha no puede asignar ese director");
          location.href="revision_fichas_director.php";</script>';
          } else {
         ...
         VALUES('$id_lista_usuario','$id_lista_ficha', '$id_rol_ficha') ";
               ...
               echo '<script> location.href="revision_fichas_director.php";</script>';
          }
     }
}
